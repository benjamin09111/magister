%% main_mo_vs_aco.m
% Comparación de Minimal Overlaps (MO) vs MO+ACO (ACO combinacional sobre MO).
% Genera los mismos gráficos que main_experiments_control.m pero comparando con MO+ACO.

clear; clc; close all;

this_file = mfilename('fullpath');
this_dir = fileparts(this_file);
% El directorio de main/other_routing/ está dos niveles abajo del root (mo_sp_pt1)
...
% Parámetros por defecto para ACO (desde config_mo_aco)
cfg.aco_num_ants = 20;
...
cfg.num_candidates_per_flow = 8;
cfg.max_candidate_attempts = 40;
...
fprintf('\nCorriendo experimento general: MO+ACO vs MO...\n');
results = run_experiment_suite_vs_mo(cfg, @run_moaco_routing, 'MO+ACO');

sched = struct();
% fprintf('\nCorriendo experimento de schedulability: MO+ACO vs MO...\n');
% sched = run_experiment_suite_schedulability_vs_mo(cfg, @run_moaco_routing, 'MO+ACO');
...
if save_results
    out_path = fullfile(project_root, 'results_mo_vs_aco.mat');
