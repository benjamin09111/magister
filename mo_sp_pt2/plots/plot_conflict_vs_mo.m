function plot_conflict_vs_mo(results, cfg)
% Figura estilo paper: Avg. worst-case conflict demand (ALT vs MO)
...
for l_idx = 1:length(results.lambdas)
    ...
    plot(ax, results.n_range, results.mean_conflict_alt(l_idx, :), ...
        'linestyle', '--', 'marker', 'o', ...
...
for l_idx = 1:length(results.lambdas)
    ...
    legend_labels{end+1} = sprintf('$\\lambda=%d$ %s', lambda_val, method_name);
