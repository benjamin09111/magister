...
**Github del Proyecto:** [https://github.com/benjamin09111/magister](https://github.com/benjamin09111/magister)

---

## 1. INTRODUCCIÓN

*Esta sección presenta de manera clara, breve y precisa la problemática de investigación, el contexto de las redes inalámbricas de sensores industriales y la justificación teórica y práctica del estudio.*

### 1.1. Contexto general del área
En el núcleo de la cuarta revolución industrial (Industria 4.0) y del Internet Industrial de las Cosas (IIoT), la automatización de procesos críticos exige infraestructuras de comunicación capaces de igualar la confiabilidad, la predecibilidad y la seguridad de las conexiones cableadas tradicionales. Históricamente, las fábricas han dependido de buses de campo cableados debido a sus estrictas garantías de tiempo real. Sin embargo, las redes de sensores y actuadores inalámbricos industriales (WSAN, por sus siglas en inglés) han irrumpido con fuerza en el sector gracias a su flexibilidad de despliegue, reducción drástica de costos de cableado y adaptabilidad en entornos móviles o de difícil acceso. 

Para satisfacer los requisitos de determinismo, confiabilidad y latencias acotadas requeridos por los sistemas de control en lazo cerrado, la subcapa MAC de los estándares inalámbricos industriales modernos emplea el mecanismo *Time-Synchronized Channel Hopping* (TSCH), normalizado por la enmienda IEEE 802.15.4e. TSCH combina el acceso múltiple por división de tiempo (TDMA) a nivel de ranuras temporales (time slots) con diversidad de frecuencia (channel hopping) sobre múltiples canales de radio activos (hasta 16 canales). Esta sincronización a nivel global de la red permite un acceso al medio predecible y analizable, proporcionando las bases para garantizar cotas de retraso en el peor de los casos (worst-case delay) y evaluar formalmente la factibilidad temporal del sistema completo (planificabilidad o *schedulability*).
...
El trabajo seminal de Wu et al. introdujo el enrutamiento consciente de conflictos (Conflict-Aware Routing) para redes fijas bajo prioridades estáticas. No obstante, las prioridades dinámicas basadas en la política Earliest Deadline First (EDF) ofrecen una utilización de espectro significativamente mayor en redes TSCH. En este contexto, Gutiérrez Gaitán et al. propusieron en el taller NG-RES 2021 la heurística de Solapamiento Mínimo (Minimal Overlaps - MO) para redes EDF. MO reduce los solapamientos de nodos re-ponderando de forma iterativa y codiciosa el peso de las aristas en el grafo del sistema. Sin embargo, persisten importantes brechas en el conocimiento y aplicación práctica de esta heurística:
...
* **Aporte Metodológico y de Replicación:** Replicación funcional del framework de estimación de demanda temporal (contención + conflicto) y simulación Monte Carlo sobre 30 topologías aleatorias con validación de planificabilidad EDF.
* **Aporte Algorítmico y Comparativo:** Unificación y evaluación comparativa directa de enfoques centralizados (MO, MO+ACO) contra esquemas basados en agentes de aprendizaje por refuerzo secuencial cooperativo (Q-Learning, SARSA) bajo un mismo dataset topológico controlado.
...
3. **Desarrollar y evaluar** algoritmos de aprendizaje por refuerzo (**Q-Learning** y **SARSA**) como esquemas de enrutamiento cooperativo secuencial para evaluar la diferencia de rendimiento frente a los enfoques centralizados.
...
El resto del informe se estructura en tres secciones principales de acuerdo al formato oficial:
La presente **Sección 1** introdujo el contexto, problema, brecha y objetivos del estudio.
La **Sección 2 (Cuerpo de la Obra)** desarrolla detalladamente el marco teórico de TSCH, EDF y el análisis formal de planificabilidad temporal, describe la metodología experimental Monte Carlo implementada en MATLAB, y presenta y discute los resultados comparativos entre los diferentes algoritmos junto con la especificación del dashboard de software.
Finalmente, la **Sección 3 (Conclusiones)** sintetiza las lecciones aprendidas de las simulaciones, propone el cronograma de trabajo futuro para el Seminario II, y lista la bibliografía de referencia de la obra.

---

## 2. CUERPO DE LA OBRA

*Esta sección constituye el núcleo del trabajo de tesis, abordando de manera exhaustiva el marco teórico de las comunicaciones industriales en tiempo real, el diseño metodológico implementado para las simulaciones, la evaluación empírica sistemática y la propuesta de transferencia tecnológica.*

### 2.1. Marco Teórico y Revisión de la Literatura

#### 2.1.1. Comunicaciones inalámbricas industriales y el estándar TSCH
...
#### 2.1.2. Planificación de paquetes EDF y análisis de planificabilidad (Schedulability)
...
$$\forall \ell \ge 0: \quad \text{Demand}(\ell) \le m \cdot \ell$$
Donde la función de oferta del canal ($\text{sbf}$) para una red con $m$ canales está acotada por $\text{sbf}(\ell) = m \cdot \ell$. La demanda de red bajo EDF en una ventana crítica de longitud $\ell$ se modela mediante el aporte de dos componentes físicos disociados:
...
#### 2.1.3. Enrutamiento en tiempo real y el problema de conflictos de transmisión
...
#### 2.1.4. El algoritmo de Solapamiento Mínimo (MO) y sus limitaciones codiciosas
...
#### 2.1.5. Optimización de Colonias de Hormigas (ACO) y el híbrido MO+ACO
...
Este enfoque híbrido aprovecha la capacidad de MO de acotar el espacio de búsqueda a rutas de excelente calidad y la capacidad de ACO de realizar una búsqueda estocástica global en la vecindad de las mejores soluciones.

#### 2.1.6. Aprendizaje por Refuerzo (Q-Learning y SARSA) en el enrutamiento
El **Aprendizaje por Refuerzo (RL)** es un área del aprendizaje automático donde un agente interactúa con un entorno y aprende a tomar decisiones óptimas (acciones) en diferentes situaciones (estados) para maximizar una recompensa acumulada. En el enrutamiento en redes de sensores, el problema se modela como un agente de enrutamiento que viaja de nodo en nodo (estados) hasta alcanzar el gateway (estado terminal).
...
**El Desafío de la Cooperación y el Enfoque Secuencial Cooperativo:**
Tradicionalmente, en su formulación independiente o "egoísta", cada agente aprende su ruta ignorando los demás flujos. Como la recompensa estándar penaliza cada salto (`R = -1`) y premia la llegada al gateway (`R = +100`), la tabla Q converge inevitablemente hacia el camino mínimo (SP), congestionando los mismos nodos de cuello de botella de la red. 

Para resolver esta limitación, en este proyecto se propuso e implementó una **estrategia de entrenamiento secuencial cooperativo**. Cuando es el turno de entrenar al sensor $i$, el agente tiene visibilidad de las rutas definitivas ya elegidas por los sensores anteriores $1$ a $i-1$. Su función de recompensa durante las caminatas estocásticas se penaliza fuertemente si transita por nodos intermedios ocupados por estas rutas previas:
$$R_i(s, a, s') = R_{\text{base}}(s') + R_{\text{hop}}(s') + R_{\text{overlap}}(s')$$
donde:
$$R_{\text{base}}(s') = \begin{cases} +100 & \text{si } s' = \text{gateway} \\ 0 & \text{en otro caso} \end{cases}$$
$$R_{\text{hop}}(s') = -1$$
$$R_{\text{overlap}}(s') = -20 \cdot \sum_{j=1}^{i-1} \mathbb{I}\Big(s' \in \phi_j \setminus \{\text{gateway}\}\Big)$$
Donde $\mathbb{I}$ es la función indicadora y $\phi_j$ es la ruta definitiva ya consolidada para el sensor anterior $j$. Esto transforma la optimización individual ciega en un proceso adaptativo donde cada agente aprende autónomamente a esquivar los cuellos de botella generados por la red ya enrutada, compitiendo de igual a igual con esquemas centralizados complejos.

#### 2.1.7. Estado de la práctica industrial e identificación de brechas en ingeniería
...
- **Sobredimensionamiento de Infraestructura:** Al no poder analizar la planificabilidad de la red, los ingenieros suelen sobredimensionar la cantidad de routers de enlace o gateways para evitar cuellos de botella de manera empírica, elevando los costos del proyecto.
...
### 2.2. Metodología de la Simulación y Diseño Experimental

#### 2.2.1. Tipo de estudio y enfoque metodológico
...
#### 2.2.2. Fuentes de datos y generación de escenarios experimentales
...
* **Control de Conectividad (Densidad del Grafo):** Para analizar el impacto de la conectividad en el enrutamiento, se varía el grado medio de los vértices ($\lambda \in \{4, 8, 12\}$). La densidad del grafo $\Lambda$ se calcula de forma directa como $\Lambda = \lambda/N$. La matriz de adyacencia del grafo se inicializa como una matriz dispersa y simétrica unida. Para asegurar la factibilidad física, si un nodo queda desconectado (aislado) durante la generación aleatoria, el framework fuerza una conexión de enlace con el nodo vecino más cercano en el espacio métrico.
* **Selección del Gateway por Centralidad:** El gateway no se selecciona al azar, sino que se define de forma estricta como el vértice del grafo que ostenta la mayor centralidad de intermediación (*betweenness centrality*). Esto de acuerdo a la topología de estrella y malla híbrida típica de WirelessHART.
* **Muestra de Simulación:** El tamaño de la muestra se unificó a **30 pruebas independientes** por escenario ($3 \times 11 \times 30 = 990$ simulaciones Monte Carlo por algoritmo) a partir del mismo dataset controlado de topologías. Esto permite mantener una significancia estadística sólida a la vez que optimiza los tiempos de cómputo para los experimentos.

#### 2.2.3. Pipeline de ejecución de la simulación
Cada trial individual dentro del framework de simulación en MATLAB sigue un flujo secuencial controlado:
1. **Generación de Tráfico Armónico ($T_i$):** Para los $n$ sensores seleccionados en el trial, se asignan períodos de muestreo armónicos $T_i$ de forma aleatoria y uniforme dentro del conjunto discreto definido por $T_i \in \{16, 32, 64, 128\}$ slots de tiempo de TSCH. Esto define el hiperperiodo global de simulación como $H = \text{mcm}(T_i) = 128$ slots. Los plazos límite se configuran como implícitos ($D_i = T_i$).
...
3. **Cálculo del Tiempo de Transmisión Efectivo ($C_i$):** El tiempo de ejecución del flujo $f_i$ libre de interferencias en la red se calcula como $C_i = 2 \cdot \text{hops}_i$ (asumiendo $w = 2$ slots por enlace).
4. **Simulación Acelerada (Enfoque en Enrutamiento):** Para comparar los métodos de ruteo de forma rápida, la ejecución se configuró para simular directamente los *overlaps* y *hops*, omitiendo la evaluación de la demanda de contención, conflicto y schedulability en las ejecuciones rutinarias de comparación de ruteo.

#### 2.2.4. Técnicas de análisis de datos y métricas de rendimiento
Los datos generados por la simulación en MATLAB se procesan estadísticamente extrayendo la media aritmética sobre los 30 trials independientes:
* **Overlaps Totales ($\Omega$):** Suma acumulada de las colisiones de nodos entre todos los pares de rutas activas de la red (excluyendo el gateway):
  $$\Omega = \sum_{i < j} \delta_{i,j}$$
* **Longitud Promedio de Rutas (Hops):** Mide el costo físico de desviar las rutas:
  $$\text{Hops}_{\text{avg}} = \frac{1}{n} \sum_{i=1}^{n} \text{hops}(\phi_i)$$
* **Tasa de Planificabilidad (Schedulability Ratio):** Mide el porcentaje de las topologías simuladas que resultan planificables en tiempo real bajo EDF. Un trial se declara planificable ($1$) si y solo si en todas las ventanas críticas posibles $\ell \le H$ se cumple la condición del test FF-DBF-WIN: $\text{Demand}(\ell) \le m \cdot \ell$. En caso contrario, se declara no planificable ($0$).

#### 2.2.5. Aspectos éticos e integridad científica
Este estudio experimental se rige bajo principios de transparencia y reproducibilidad:
* **Reproducibilidad y Código Abierto:** Todo el código fuente de las simulaciones en MATLAB (incluyendo los scripts, wrappers de enrutamiento y funciones de graficado) se mantiene en el repositorio público de GitHub ([https://github.com/benjamin09111/magister](https://github.com/benjamin09111/magister)).
* **Evitación del Sesgo de Selección (Semillas Fijas):** Se emplean generadores de números pseudoaleatorios con semillas fijas basadas en el identificador de cada trial para garantizar reproducibilidad exacta.

---

### 2.3. Resultados Experimentales y Discusión Comparativa

#### 2.3.1. Análisis del solapamiento de nodos ($\Omega$)
...
* **Shortest Path (SP):** Presenta el peor desempeño en todos los escenarios. Al enrutar egoístamente por longitud de saltos, los flujos convergen sobre los mismos nodos de paso intermedio y el gateway, provocando un aumento exponencial de solapamientos a medida que crece el número de flujos ($n$).
* **Minimal Overlaps (MO):** Logra mitigar drásticamente los solapamientos. Al penalizar de forma codiciosa los nodos compartidos, MO desvía las rutas y reduce las colisiones a menos de la mitad en comparación con SP.
...
#### 2.3.2. Impacto en la longitud promedio de las rutas (Hops)
...
* **MO y MO+ACO:** Ambos algoritmos incrementan la longitud de las rutas de manera marginal. El desvío promedio es despreciable (un incremento de menos de $0.2$ saltos por flujo en promedio respecto a SP, situándose en $\sim 2.78$ saltos).
* **Q-Learning y SARSA:** Registran un incremento de saltos moderado (entre $0.4$ y $0.8$ saltos adicionales respecto a SP). Al verse obligados a rodear las zonas congestionadas debido a la penalización por colisión incorporada en la señal de recompensa, los agentes de aprendizaje por diferencia temporal encuentran desvíos alternativos de manera autónoma, logrando un balance eficiente entre la evasión de solapamientos y el aumento de la latencia de red.

#### 2.3.3. Demanda por conflictos y contención de canal
* **Demanda de Contención:** Es idéntica o muy similar para todos los métodos, ya que depende principalmente del número total de slots requeridos para transmitir los paquetes ($2 \cdot \text{hops}$).
...
#### 2.3.4. Tasa de planificabilidad temporal (Schedulability Ratio)
...
2. **Desempeño de Q-Learning y SARSA:** En su versión egoísta aislada original, los enfoques descentralizados obtenían tasas de planificabilidad deficientes al saturar nodos centrales. No obstante, con la incorporación del entrenamiento secuencial cooperativo con penalización de colisiones en la recompensa, la tasa de planificabilidad del sistema mejora de manera notable, logrando planificar con éxito redes densas bajo planificación EDF y validando la viabilidad de utilizar aprendizaje por refuerzo tabular adaptativo.
...
#### 2.3.5. Evaluación Paramétrica Complementaria de la Red

##### Análisis de Variaciones en la Selección del Gateway
En la fase experimental se evaluó el impacto estructural de la selección del colector central (gateway). Se evaluaron cuatro métricas clásicas de centralidad en grafos para definir el nodo pasarela:
1. **Degree Centrality (Centralidad de Grado):**
   $$C_D(v) = \text{deg}(v)$$
2. **Closeness Centrality (Cercanía):**
   $$C_C(v) = \frac{N-1}{\sum_{u \neq v} d(v, u)}$$
3. **Eigenvector Centrality (Vector Propio):**
   $$x_v = \frac{1}{\lambda} \sum_{u \in N(v)} x_u$$
4. **Betweenness Centrality (Intermediación):**
   $$C_B(v) = \sum_{s \neq v \neq t} \frac{\sigma_{st}(v)}{\sigma_{st}}$$

Los resultados demuestran que seleccionar el gateway basado en **Betweenness Centrality** genera la mayor propensión a solapamientos de caminos mínimos en SP, ya que el gateway se sitúa en el centro geométrico de los flujos de tránsito del grafo. No obstante, este posicionamiento centralizado es el que ofrece la menor longitud de saltos promedio para los sensores. Al emplear **MO o MO+ACO** sobre topologías con gateway por intermediación, la tasa de planificabilidad mejora drásticamente en comparación a centralidades basadas únicamente en grado o cercanía, puesto que los algoritmos de enrutamiento conscientes de colisiones explotan de manera eficiente las rutas alternativas periféricas distribuyendo la carga de tráfico de forma radial.

##### Análisis de Sensibilidad Paramétrica de la Densidad de Penalización $\psi$ (Valor Phi)
El parámetro $\psi$ en el algoritmo MO determina la magnitud de la penalización de peso aplicada a las aristas incidentes en nodos colisionados:
$$w_{u,v}^{(k)} = w_{u,v}^{(k-1)} + \delta_{i,j} \cdot \psi$$
donde por defecto se define como la densidad del grafo ($\psi = \lambda / N$). En las simulaciones de calibración paramétrica se identificaron dos comportamientos límites:
* **Sub-penalización ($\psi \to 0$):** El incremento de peso en las aristas es insignificante. El algoritmo Dijkstra no se ve incentivado a buscar desvíos y MO degenera en el comportamiento de Shortest Path (peor versión de MO, *worst MO*), con solapamientos máximos.
* **Sobre-penalización ($\psi \to \infty$):** Incluso un solapamiento leve genera un costo de arista prohibitivo. Los flujos se desvían excesivamente recorriendo caminos periféricos masivos, lo que incrementa sustancialmente la longitud promedio de saltos ($\text{Hops}_{\text{avg}}$) y la demanda por contención de canal, reduciendo la tasa de planificabilidad temporal global.
* **Punto de Equilibrio Óptimo:** La escala adaptativa propuesta basada en el grado medio sobre el tamaño del grafo ($\psi = \lambda / N$) proporciona el balance ideal (*best MO*), forzando desvíos localizados estrictamente necesarios para anular los solapamientos sin incurrir en un incremento perjudicial de saltos físicos.

---

### 2.4. Especificación del Software de Visualización Propuesto (Trabajo Futuro)
...
#### 2.4.1. Arquitectura conceptual del sistema
...
```
* **Capa de Presentación (Front-end):** Una aplicación web de una sola página (SPA) construida en React.js, utilizando la biblioteca Cytoscape.js o D3.js para el renderizado dinámico de grafos interactivos.
* **Capa de Lógica de Negocio (Back-end):** Un servidor web basado en Python (FastAPI). Esta capa ejecuta las funciones de cálculo matemático de demanda de contención y conflicto, y contiene los motores de los algoritmos de enrutamiento (SP, MO, MO+ACO, Q-Learning, SARSA).
...
#### 2.4.2. Módulos funcionales del software
El dashboard contará con cuatro módulos principales accesibles desde una interfaz web con un diseño estético premium (modo oscuro, gradientes armoniosos y diseño responsivo):
1. **Módulo de Carga y Configuración Topológica:** Permite al usuario importar topologías a través de matrices de adyacencia o dibujar de forma gráfica los nodos y enlaces sobre un plano digital de la planta, seleccionando el gateway y sensores, asignando sus períodos ($T_i$) y plazos ($D_i$).
2. **Módulo de Ejecución y Selección de Enrutamiento:** Panel lateral para elegir qué algoritmo ejecutar sobre la topología cargada y configurar parámetros de simulación (número de canales de radio $m$, número de retransmisiones $w$, etc.).
3. **Módulo de Visualización Estructural del Grafo:** Renderiza el grafo de la planta de manera interactiva. Los nodos y aristas compartidos por múltiples rutas se tiñen de colores de calor (rojo, naranja) para indicar visualmente el nivel de solapamiento y conflicto. Las rutas limpias se tiñen de verde.
4. **Módulo de Reportes y Análisis Temporal:** Despliega gráficos interactivos de la Demand Bound Function frente a la función de oferta de canal ($\text{sbf}$) para cada ventana crítica. Muestra de forma destacada una alerta binaria: **APROBADO (Planificable)** en verde o **RECHAZADO (Pérdida de plazos en el peor caso)** en rojo, indicando el instante exacto de la falla.

---

## 3. CONCLUSIONES

*Este capítulo final consolida los hallazgos empíricos y teóricos obtenidos, evalúa críticamente los logros del estudio frente a los objetivos, detalla el cronograma de trabajo e hitos planificados para el Seminario II y presenta la bibliografía formal de la investigación.*
...
* **Fidelidad en la Replicación:** Se logró replicar con absoluta fidelidad y reproducibilidad científica el framework de simulación Monte Carlo y de estimación de demanda temporal del paper base NG-RES 2021. Los resultados validan que la heurística de Solapamiento Mínimo (MO) domina de forma consistente a caminos mínimos (SP) en todos los escenarios de congestión y densidad inalámbrica.
* **Éxito de la Optimización Híbrida (MO+ACO):** La implementación de la metaheurística de colonia de hormigas **MO+ACO** superó con éxito los mínimos locales del comportamiento puramente codicioso de MO. Al optimizar probabilísticamente la vecindad combinatoria global de rutas, logró una reducción sustancial de solapamientos ($\Omega$) en escenarios congestionados, lo que se tradujo en una tasa de planificabilidad EDF superior (hasta 15% de holgura extra de éxito temporal).
...
### 3.2. Trabajo futuro (Seminario II)
...
4. **Exploración de los Límites de la Densidad de Penalización $\psi$ (Valor Phi):**
   ...
   - Realizar simulaciones barriendo un espectro continuo de valores para $\psi$ para modelar y definir formalmente los límites de rendimiento del algoritmo de Solapamiento Mínimo. Esto permitirá contrastar las metaheurísticas de búsqueda global frente a una versión de mejor desempeño (*best MO*) y una versión degradada (*worst MO*) basadas en la elección de este parámetro, aportando un marco de comparación teórica mucho más robusto.
